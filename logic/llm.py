...
def generate_ai_career_overview(role_title: str) -> Optional[Dict]:
    """
    Generates a structured career overview JSON if local data is missing.
    """
    if not client:
        return None

    prompt = f"""
    Generate a career overview for the role: "{role_title}" for the year 2026.
    Return ONLY a valid JSON object with the following keys:
    - 'description': Brief explanation of what the role does.
    - 'why_now': Why this role matters in the heavily automated 2026 AI era.
    - 'core_skills': An array of strings representing fundamental skills to learn.
    - 'emerging_skills': An array of strings for cutting-edge / AI-native skills for this role.
    ...
    {{
      ...
      "emerging_skills": ["AI Tool C"]
    }}
    
    Be brutally realistic and specific.
    """
    try:
        completion = client.chat.completions.create(
            model=MODEL,
        ...
        logger.error(f"Error generating AI career overview: {e}")
        return None

def generate_detailed_description(role_title: str) -> str:
    """
    Generates a dense, 5-line minimum description avoiding fillers.
    """
    if not client:
        return ""
    prompt = f"Provide a highly informative, brutally realistic professional breakdown (minimum 5 dense sentences) of what a {role_title} actually does day-to-day. Do not use generic filler words like 'In today's fast paced world'. Focus strictly on tactical responsibilities, core workflows, and the exact business value they provide in the modern AI-integrated landscape. IMPORTANT: Format your response into 2-3 distinct, easy-to-read paragraphs. Separate the paragraphs with empty lines."
    try:
...
def generate_detailed_why_now(role_title: str) -> str:
    """
    Generates a detailed, brutally realistic 2026 reality check for a career path.
    """
    if not client:
        return ""
    prompt = f"Provide a detailed, brutally realistic insight (minimum 4 dense sentences) explaining exactly why the {role_title} role is critical, evolving, or at risk in the 2026 AI era. Explain how AI acts as a multiplier or displacer for this path. Do not use generic filler words. Format your response into 1-2 distinct paragraphs separated by empty lines."
    try:
        res = client.chat.completions.create(
            model=MODEL,
            messages=[{"role": "user", "content": prompt}],
